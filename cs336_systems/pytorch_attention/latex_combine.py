...
def combine_and_save_latex():
    ...
    if dfs:
        ...
        combined_df = combined_df.drop(columns=['num_warmups'])
        combined_df = combined_df.drop(columns=['forward_times_std'])
        combined_df = combined_df.drop(columns=['backward_times_std'])

        combined_df.columns = ['Model', 'Forward (mean)', 'Backward (mean)', 'Memory Usage', 'Compile Model', 'Compile Attention', 'D Model', 'Seq Len']

        # reorder the columns to the following
        combined_df = combined_df[['Model', 'Compile Model', 'Compile Attention', 'D Model', 'Seq Len', 'Forward (mean)', 'Backward (mean)', 'Memory Usage']]
        ...
        combined_df = combined_df.sort_values(['D Model', 'Seq Len'])
        ...
        combined_df_0 = combined_df[combined_df['Compile Model'] == 0]
        combined_df_0 = combined_df_0[combined_df_0['Compile Attention'] == 0]
        # combined_df_0.to_csv(f"table_nocompile.csv", index=False)
        combined_df_0.to_latex(f"table_nocompile.txt", index=False)
    else:
